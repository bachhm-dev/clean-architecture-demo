...
func (m *MockWeatherRepository) GetWeatherFromCache(ctx context.Context, latitude, longitude float64) (*entity.Weather, error) {
	args := m.Called(ctx, latitude, longitude)
	return args.Get(0).(*entity.Weather), args.Error(1)
}

func (m *MockWeatherRepository) SaveWeatherToCache(ctx context.Context, latitude, longitude float64, weather *entity.Weather) error {
...
func TestWeatherServiceGetWeatherCacheHit(t *testing.T) {
	...
	mockRepo.On("GetWeatherFromCache", mock.Anything, 10.0, 20.0).Return(expectedWeather, nil)
...
func TestWeatherServiceGetWeatherCacheMiss(t *testing.T) {
	mockRepo := new(MockWeatherRepository)
	mockMeteoService := new(MockOpenMeteoService)
	service := NewService(mockRepo, mockMeteoService)
	expectedWeather := &entity.Weather{Temperature: 25.0}
	mockRepo.On("GetWeatherFromCache", mock.Anything, 10.0, 20.0).Return(nil, errors.New("cache miss"))
	mockMeteoService.On("GetWeather", mock.Anything, 10.0, 20.0).Return(expectedWeather, nil)
	mockRepo.On("SaveWeatherToCache", mock.Anything, 10.0, 20.0, expectedWeather).Return(nil)
	...
	assert.Equal(t, expectedWeather, weather)
	mockRepo.AssertExpectations(t)
	mockMeteoService.AssertExpectations(t)
}

func TestWeatherServiceGetWeatherOpenMeteoServiceError(t *testing.T) {
	mockRepo := new(MockWeatherRepository)
	mockMeteoService := new(MockOpenMeteoService)
	service := NewService(mockRepo, mockMeteoService)

	mockRepo.On("GetWeatherFromCache", mock.Anything, 10.0, 20.0).Return(nil, errors.New("cache miss"))
	mockMeteoService.On("GetWeather", mock.Anything, 10.0, 20.0).Return(nil, errors.New("service error"))
